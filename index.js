...
app.get("/", async (req, res) => {
    try {
        const sortOption = req.query.sort; // Changed to req.query to handle GET request parameters
        let books = [];
        ...
        if (sortOption === "rating") {
            ...
            const bookInfo = await db.query("SELECT * FROM books ORDER BY id DESC");
            books = bookInfo.rows;
...
app.post("/new", async (req, res) => {
    try {
        ...
        let date = new Date().toISOString().split('T')[0]; // Outputs: 2024-07-05
...
app.post("/delete", async (req, res) => {
    const { title, description, author, rating, isbn } = req.body; // Assuming `isbn` is part of the request body
